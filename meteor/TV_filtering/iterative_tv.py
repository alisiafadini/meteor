...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "MTZ to be used for initial map. Specified as (filename, F_off, F_on, Phi)"
        ),
    )
...
def main():
    ...
    # og_mtz = og_mtz.loc[og_mtz.compute_dHKL()["dHKL"] < 15]

    # for ocp
    # calc = io.load_mtz("ecn_dark_SFALL.mtz")
    # common = og_mtz.index.intersection(calc.index).sort_values()
    ...
    if args.flags is not None:
        ...
        og_mtz = og_mtz.loc[
            :, [args.mtz[1], args.mtz[2], args.mtz[3], args.mtz[4], args.mtz[5]]
        ]
    ...
    # og_mtz["FC"] = calc.loc[common, "FC_ALL"]
    og_mtz, ws = maps.find_w_diffs(
        og_mtz,
        args.mtz[2],
        args.mtz[1],
        args.mtz[5],
    ...
    with tqdm(total=N) as pbar:
        for i in np.arange(N) + 1:
            if i == 1:
                new_amps, new_phases, proj_mag, entropy, phase_change, z = (
                    tv.TV_iteration(
                        og_mtz,
                        "WDF",
                        args.mtz[3],
                ...
                cum_phase_change = np.abs(
                    np.array(
                        dsutils.positive_Fs(
                            og_mtz, args.mtz[3], "WDF", "phases-pos", "diffs-pos"
                        )["phases-pos"]
                        - new_phases
                    )
                )
    ...
    og_mtz["new_amps"] = new_amps * ws

    finmap = dsutils.map_from_Fs(og_mtz, "new_amps", "new_phases", map_res)

    from meteor import validate

    fin_map_afterkweight = validate.negentropy(np.array(finmap.grid).flatten())
    print("FINAL ENTROPY AFTER K WEIGHTING ", fin_map_afterkweight)
    finmap.write_ccp4_map("{name}_TVit{i}_{l}.ccp4".format(name=name, i=i, l=l))
    fin_TV_map, fin_ent_TV = tv.TV_filter(
        finmap, l, finmap.grid.shape, cell, space_group
    )
    print("FINAL TV FILTERED ENTROPY VALUE", fin_ent_TV)
    mtz_finTV = dsutils.from_gemmi(io.map2mtz(fin_TV_map, high_res))
    mtz_finTV.write_mtz(
        "{name}_TVit{i}_{l}_finalTVfiltered.mtz".format(name=name, i=i, l=l)
    )
    ...
    if args.plot is True:
        ...
        ax1.plot(
            np.arange(N - 1),
            np.array(
                np.mean(np.array(proj_mags), axis=1)
                / np.max(np.mean(np.array(proj_mags), axis=1))
            )[1:],
            color=color,
            linewidth=5,
        )
        ...
        # np.save("/Users/alisia/Desktop/negentropy.npy", entropies[1:])
        ...
        # np.save("/Users/alisia/Desktop/non-cum-phi-change.npy", phase_changes)
        ...
        ax.plot(
            np.arange(N),
            np.mean(cum_phase_changes, axis=1),
            color="orangered",
            linewidth=5,
        )
        ...
        # np.save(
        #    "/Users/alisia/Desktop/cum-phi-change.npy",
        #    np.mean(cum_phase_changes, axis=1),
        # )
        ...
        ax.scatter(
            1 / og_mtz.compute_dHKL()["dHKL"][flags],
            proj_mags[N - 1],
            color="black",
            alpha=0.5,
        )
        ...
        res_mean, data_mean = dsutils.resolution_shells(
            np.abs(cum_phase_changes[N - 1]), 1 / og_mtz.compute_dHKL()["dHKL"], 15
        )
