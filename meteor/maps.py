...
def find_w_diffs(mtz, Fon, Foff, SIGon, SIGoff, pdb, high_res, path, a, Nbg=1.00):
    ...
    mtx_on, t_on, scaled_on     = scale.scale_aniso(np.array(calcs), np.array(mtz[Fon]), np.array(list(mtz.index)))
    mtx_off, t_off, scaled_off  = scale.scale_aniso(np.array(calcs), np.array(mtz[Foff]), np.array(list(mtz.index)))
...
def screen_alpha_weight(data, F_on, F_off, SIGF_on, SIGF_off, philabel, refpdb, path, name, map_res, center, radius=8.0,hres=None, percent=0.03, flags=None) :
    ...
    if flags is not None:
        _, _, choose_test = validate.make_test_set(data, percent, F_off, name, path, flags) #choose_test is Boolean array to select free (test) reflections from entire set
    ...
    noweight, _   = find_w_diffs(data, F_on, F_off, SIGF_on, SIGF_off, refpdb, hres, path, 0.0)
    noweight_pos  = dsutils.positive_Fs(noweight, philabel, "DF", "ogPhis_pos", "ogFs_pos")
    ...
    for a in tqdm(alphas):
        diffs, _   = find_w_diffs(data.copy(deep=True), F_on, F_off, SIGF_on, SIGF_off, refpdb, hres, path, a)
        ...
        if hres is not None:
            ...
            diffs_all = dsutils.from_gemmi(io.map2mtz(fit_map, np.min(noweight.compute_dHKL()["dHKL"])))
